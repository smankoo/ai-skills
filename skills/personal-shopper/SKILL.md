...
|---|---|---|
| A season of clothes for one or more people | **Wardrobe** (§3–§7) | A checklist, then one cart per person |
| One outfit for a dated event | **Occasion** (§3–§7, small) | One cart, options, deadline-driven |
| A gift for someone | **Gift** (§8) | 3–5 distinct *directions*, each costed |

All three share: profile the person (§1–§2), browse and verify (§9), build and send (§10–§11).
...
wrong thing. In the 2026 gift run, asking surfaced that the recipient does standup comedy, which
no amount of database mining had revealed and which redefined every option.
...
of the gift. In the 2026 run the graph had only "Born 1986" in a notes field, which made the
recipient's 40th easy to overlook entirely.
...
## 8. Gifts: find several directions, then recommend one
...
**Sizing the gift.** Set the band from relationship, occasion, and milestone status, then confirm
with the user. Sibling-in-law milestone birthday ≈ $75–150; close family non-milestone less; a
landmark or split-across-the-family gift more. Make **every** direction land inside the band on its
own, so the user can pick freely. Then say which you'd choose.
...
**Say what you'd avoid, and why.** "A generic milestone-40 gift — whisky stones, engraved barware —
is what you buy someone whose interests you don't know. You know his." That sentence is worth more
than a sixth option.

## 9. Browse and verify — the part that actually takes the time
...
- **Made-to-order and personalised items are the trap.** A custom item can show "in stock" and
  still quote delivery *weeks* out — one custom mic flag said "ships in 4–5 days" and quoted
  31 Aug–21 Sep against a 9 Aug deadline. Always read the actual date.
...
- **Check a pickup fallback** when delivery is tight — Best Buy/Walmart/Indigo often stock the
  same item for same-day pickup, sometimes at a slightly different price or a newer model. Quote
  both, with the price difference.
...
## 10. Write one markdown file per person
...
For a **gift**, the file is one per occasion (`<name>-40.md`): what you learned about the person and
where it came from, the five directions with their own tables and per-direction totals, the
recommendation, what you'd avoid, and anything adjacent worth doing (a related event to attend, a
missing record worth fixing).
...
set to a hand-added $561.61 when the real sum was $510.64, and the build failed loudly instead of
sending a wrong number. Take the computed figure, sanity-check it, and use it.
...
- `references/example-run-2026-fall.md` — the wardrobe run: 4 people, 73 items, $3,623.04, with
  the sizing derivations, the graduation follow-on, and each error caught along the way.
- `references/example-run-2026-gift.md` — the gift run: a milestone 40th, five directions, and how
  the deciding fact came from asking the user rather than from the graph.

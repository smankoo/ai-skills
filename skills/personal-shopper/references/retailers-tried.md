...
| Hatley | CA | Shopify `/products/<handle>.js` (price/stock CENTS CAD, per-size `variants[].available`, image) + PDP-HTML `<strong>Materials</strong><br>` composition; `hatley_extract.py`. NO bot wall, pure `urllib` VPS-side. Montreal brand: kids' 100% cotton PJs/tees/playwear + adult tops (fits whole household). ⚠️ "bamboo" PJs = viscose (synthetic); rain jackets = 100% poly (fail gate) | done | yes | 2026-09-03 |
...
| mini mioche | CA | Shopify `/products/<handle>.js` ALONE (price cents CAD + per-colour/size `variants[].available` + image + composition in `description` prose); `minimioche_extract.py`. NO bot wall, pure `urllib` VPS-side. Toronto DTC, **100% GOTS organic cotton, made in Canada, WHOLE household** (newborn→14y→men/women) — safest natural-fibre store found. Prices already CAD (NOT USD); JSON-LD availability reflects default colour only → trust `.js` per-variant | done | yes | 2026-09-04 |
| Muttonhead / MTN.HEAD | CA | Shopify `/products/<handle>.js` ALONE (price cents CAD + per-size `variants[].available`+`inventory_quantity` + image + composition in `description` prose via fibre-whitelist regex); `muttonhead_extract.py`. NO bot wall, pure `urllib` VPS-side. Toronto made-in-CA unisex+kids; house line heavy 100% (organic/combed) cotton + resells Naked&Famous denim at CAD. ⚠️ `muttonheadstore.com` 301→`www.mtnhead.com`; some house tees are poly-cotton-rayon tri-blends (fail 70% gate) — read the % | done | yes | 2026-09-04 |
| Pact / wearpact.com | US (ships CA) | Undocumented public JSON API `GET api2.wearpact.io/product/search?sku=<styleCode>` — one call = per-size price+`quantityAvailable`+`fabricContent`+image; `pact_extract.py`. NO bot wall, pure `urllib` VPS-side. wearpact.com PDP is a hydrated shell (empty JSON-LD, no price); `/product/*` on wearpact.com 302→`/`. Top organic-cotton source (women/men/kids/baby/home). ⚠️ prices always USD; `&country=CA` only filters sizes; "Cool Stretch"/movement = 95% cotton/5% elastane | done | yes | 2026-09-05 |
| Kit and Ace | CA | Shopify `/products/<handle>.js` (price/compare CENTS **CAD**, per-size `variants[].available`, image) + static PDP "Fabric • Care" accordion (`<div class="accordion_details"><p>NN% …`) composition; `kitandace_extract.py`. NO bot wall, pure `urllib` VPS-side. Vancouver premium DTC (men+women): cotton-cashmere/merino/silk knits — natural-fibre-friendly. ⚠️ `.js` currency null & `?currency=` ignored → store is CA-default, treat cents as CAD; `suggest.json` price is DOLLARS not cents; "Tech Tricot"/stretch styles run synthetic — read the % | done | yes | 2026-09-05 |
...
- **Pact** (wearpact.com) — ✅ `done` (see recipe). US organic-cotton DTC (ships CA); GOTS organic cotton across women/men/kids/baby/home — top natural-fibre source. Undocumented public JSON API (`api2.wearpact.io/product/search?sku=<styleCode>`), NO bot wall, pure `urllib` VPS-side. ⚠️ prices always USD (converts at global-e checkout); "Cool Stretch"/movement lines = 95% cotton/5% elastane.
- **Hatley** (hatley.com) — ✅ `done` (see recipe). Montreal brand; kids' 100% cotton PJs/tees/playwear + adult tops — natural-fibre-friendly, whole-household. Shopify, NO bot wall, pure `urllib` VPS-side (`.js` price/stock + PDP `<strong>Materials</strong>` composition). ⚠️ "bamboo" PJs = viscose (synthetic); iconic rain jackets = 100% poly (fail gate).
...
- **mini mioche** (minimioche.com) — ✅ `done` (see recipe). Toronto DTC, **100% GOTS organic cotton, made in Canada, whole household** (newborn→14y + men/women) — the safest natural-fibre store found; single-material catalog. Shopify, NO bot wall, pure `urllib` VPS-side (`.js` alone has price/stock/image + composition in prose). ⚠️ prices already CAD (NOT USD); JSON-LD availability = default colour only → trust `.js` per-variant.
- **Muttonhead / MTN.HEAD** (mtnhead.com, was muttonheadstore.com) — ✅ `done` (see recipe). Toronto made-in-Canada unisex + kids/baby; house line heavily 100% (organic/combed) cotton + resells Naked & Famous denim at CAD list. Shopify, NO bot wall, pure `urllib` VPS-side (`.js` alone: price/stock/image + composition in prose via fibre-whitelist). ⚠️ old `muttonheadstore.com` 301→`www.mtnhead.com`; some house "cotton" tees are poly-cotton-rayon tri-blends (fail 70% gate) — read the %.
...
- **Kit and Ace** (kitandace.com) — ✅ `done` (see recipe). Vancouver premium DTC (men+women); cotton-cashmere/merino/silk knits, natural-fibre-friendly. Shopify, NO bot wall, pure `urllib` VPS-side (`.js` price/stock/image CAD + static "Fabric • Care" accordion composition). ⚠️ `.js` currency null & `?currency=` ignored (store is CA-default → CAD); `suggest.json` price is dollars not cents.
